...
static void led_2_task(void)
{
    ...
    {
        if (Motion_IsACStucked()) User_PWME_SetWidth(PWM_PulseWidth_MAX);
        else User_PWME_SetWidth(SineTable[sine_index]);
    }
    else if (Motor_GetState() == Motor_State_AUTOFOLLOW)
    {
        if (Motion_IsAFAlone()) User_PWME_SetWidth(PWM_PulseWidth_MAX);
        else User_PWME_SetWidth(SineTable[sine_index]);
    }
    else
    {
