...
/**
 * 定时器模式 0（16 位自动重装载）的时钟输出频率 f_OUT = f_溢出 / 2 = MAIN_FOSC/(65536-ARR)
 * 即 ARR = 65536 - (MAIN_FOSC / 2 / f_OUT)
 * 
 */
#define ARRT(__f)			((u16)((65536.0f - MAIN_Fosc / 2 / (__f)) + 0.50f))

/**
 * @brief 频率 - ARR 重装值对照表。
 * 
 */
static u16 code FreqTab[] =
{
